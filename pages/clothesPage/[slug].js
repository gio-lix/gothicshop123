...
export default function Slug({data, randomData}) {
    ...
    return (
        <Layout title={`${data.slug}`}>
            {infoText && (
                <div className='w-36 z-50 h-10 bg-input fixed top-16 right-0 flex items-center justify-center'>
                    <p>{infoText}</p>
                </div>
            )}
            ...
            <div
                ...
                className='w-full flex flex-col lg:flex-row justify-around content-between sm:px-32 py-4'
            >
            ...
            <div className='grid grid-cols-2 sm:grid sm:grid-cols-6   sm:px-32 gap-x-3 pb-4'>
                {randomData && (
                    Object.values(randomData).slice(0, 6).map((item, i) => (
                        <div key={i}>
...
export const getServerSideProps = async ({query: {slug}}) => {
    const {data} = await axios.get(`${API_URL}/clothes?slug=${slug}`)



    const {data: clothes} = await axios.get(`${API_URL}/clothes`)
    const randomData = clothes.reduce((acc, item) => {
        return {...acc, [Math.floor(Math.random() * clothes.length)]: item}
    }, {})

    return {
        props: {data: data[0], randomData}
    }
}