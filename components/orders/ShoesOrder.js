                ...
                <div className='h-full flex justify-between items-center p-2'>
                    <div className='flex flex-col sm:flex-row'>
                        <div className=' w-16 h-16 sm:w-24 sm:h-24 sm:w-36 sm:h-36'>
                            <Image src={item.shoes.image[0].formats.thumbnail.url || item.clothes.image[0].formats.thumbnail.url} height={100} width={100}
                                   layout='responsive'/>
                        </div>
                        <div className='sm:ml-10'>
                            <p className='text-xs sm:text-base'>{item.shoes.title}</p>
                            <p className='text-xs sm:text-base'>{item.shoes.brand}</p>
                            {/*<p>{item.totalPrice}</p>*/}
                            {/*    show chosen size*/}
                            <p>size: {item.size}</p>
                        </div>
